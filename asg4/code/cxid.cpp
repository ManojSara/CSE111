...
void reply_ls (accepted_socket& client_sock, cxi_header& header) {
   ...
   outlog << "sent LS, server returned properly" << endl;
   send_packet (client_sock, &header, sizeof header);
...
void reply_get (accepted_socket& client_sock, cxi_header& header) {
   ifstream getfile(header.filename, ifstream::binary);
   if (!getfile.is_open()) {
      ...
      int status = stat (header.filename, &stat_buf); // DO MORE
      if (status == -1) {
         outlog << "get: " << header.filename << ": "
                << strerror (errno) << endl;
         header.command = cxi_command::NAK;
      ...
      getfile.read (buffer.get(), ntohl(header.nbytes));
      getfile.close();
      header.command = cxi_command::FILEOUT;
      outlog << "sent GET, server returned properly" << endl;
      send_packet (client_sock, &header, sizeof header);
      send_packet (client_sock, buffer.get(), ntohl(header.nbytes));
   }
}

void reply_put (accepted_socket& client_sock, cxi_header& header) {
   ofstream putfile(header.filename, ofstream::binary);
   size_t host_nbytes = ntohl (header.nbytes);
   auto buffer = make_unique<char[]> (host_nbytes);
   recv_packet (client_sock, buffer.get(), host_nbytes);
   putfile.write (buffer.get(), host_nbytes);
   putfile.close();
   header.command = cxi_command::ACK;

   outlog << "sent PUT, server returned properly" << endl;
   send_packet (client_sock, &header, sizeof header);
}
...
void reply_rm (accepted_socket& client_sock, cxi_header& header) {
   ...
   if (rc == -1) {
      ...
      header.command = cxi_command::ACK;
      outlog << "sent RM, server returned properly" << endl;
      send_packet (client_sock, &header, sizeof header);
   }
}
